...
export default function SalesHistoryPage() {
  ...
  // 🔥 FIX #1: Fonction pour voir le PDF dans un modal
  const handleViewPDF = async (saleId: string) => {
    try {
      const pdfBlob = await generateBLPDF(saleId, supabase, "blob");
  ...
  // 🔥 FIX #1: Handler pour clic sur référence (ouvre le modal de détails)
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[auto_1fr_1fr_auto_auto] gap-3 items-end">
            ...
            <select
              ...
              className="px-4 py-2.5 border border-gray-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 min-h-[48px] text-base"
            >
          ...
          <div className="lg:hidden space-y-3">
            {displayedSales.map((sale) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <button
                      ...
                      className="font-bold text-blue-600 dark:text-blue-400 text-sm hover:underline text-left"
                    >
                      {sale.reference}
                    </button>
                    {sale.is_void && (
                ...
                <div className="flex justify-between items-center mb-3 pt-3 border-t border-gray-200 dark:border-slate-700">
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Vendeur</th>
      ...
      {pdfModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative w-full max-w-6xl bg-white dark:bg-slate-800 rounded-lg shadow-2xl max-h-[90vh] flex flex-col">
              <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-700">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Aperçu PDF</h3>
                <button
                  onClick={() => {
                    setPdfModalOpen(false);
                    URL.revokeObjectURL(pdfUrl);
