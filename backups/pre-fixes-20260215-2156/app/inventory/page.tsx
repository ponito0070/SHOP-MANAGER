...
export default function InventoryPage() {
  ...
  const totalStockValue = filteredProducts.reduce((sum, p) => sum + (p.stock_actuel * p.prix_achat), 0);
              ...
              <div key={product.id} className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm p-4 border-l-4 ${
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-900 dark:text-white">{product.prix_achat.toFixed(2)}</div>
                  </div>
                  <div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-slate-900/50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Prix Achat</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                    ...
                    <td className="px-6 py-4 text-right text-sm">{product.prix_achat.toFixed(2)} DA</td>
                    <td className="px-6 py-4 text-right text-sm font-medium text-green-600">{product.prix_vente.toFixed(2)} DA</td>
                    <td className="px-6 py-4 text-right font-semibold text-blue-600">{(product.stock_actuel * product.prix_achat).toFixed(2)} DA</td>
                    <td className="px-6 py-4 text-center">
